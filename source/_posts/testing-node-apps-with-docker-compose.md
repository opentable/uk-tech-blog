---
layout: post
title: testing-node-apps-with-docker-compose
date: 2016-11-09 16:40:22
author: federicomaffei
tags: [Docker, Node, TDD, Compose]
---

# Testing Node.js apps with docker-compose (and some Soul)
...
As I am sure the audience of this post knows to some extent, [Docker](https://www.docker.com/what-docker) is a technology that has grown to become popular over the last years, allowing to deploy pieces of software by packaging them into standardized containers, in a number of various ecosystems (Apache Mesos, Amazon Web Services and many more).
...
Specifically, with this post, I aim to dig deeper into how to use Docker Compose (a specific Docker-based tool that allows to create multi-container Docker applications) to build and run a Node.js application, connected to a MongoDB instance, and test their interaction and the interaction of the app with the external world, all inside containers running on your machine. All isolated and testable thanks to the usage of containers that we can spin up, hit with tests, and cleaned up with little effort. 

Interested? Let’s go!

## Scenario

More specifically, we can use Docker and one of its functionalities, [docker-compose](https://docs.docker.com/compose/), to build a container and spin up our app, then another container with a copy of the database where we can freely create and manipulate data, and finally perform all the integration testing we want against those self-contained entities, that we can also clean up after the tests ran. Total isolation and, very importantly, no need to pollute our development or pre-production environment with superfluous test data.

Let’s imagine an app that we can build and test, let’s say a directory of soul music artists.
...
This first step is not the most crucial one for this blog post’s sake, so I will not delve into it too much. I have pushed everything into a Github repo, so you will be able to see how the code should look like at each step. We will use the [hapi.js](http://hapijs.com/) framework to create a basic server application. Here is the [package.json](https://github.com/federicomaffei/soul-compose/blob/78400f40606c032fb01542d35e579dc851d82fb3/package.json) file of the Soul Compose app with all the dependencies you need to get started. Copy it in your home folder and run:
...
There is already something we could improve here. We are hitting the ‘real’ app with our request. This is not what we call isolation, right?

Good time to pull Docker in. We can use docker-compose up to build a container with our app that runs on port 3001, nice and separated from our development one. Let’s do it.
...
Let’s take a look at what this set of instructions means. We are building a container with our app, taking the base image from the Node.js official image (the environment where the app will run), exposing the port that the app will use for serving requests, and running the server at last. This, by the way, is a pretty standard way of using a Node.js within Docker.
Now, to spin up the app and hit it with tests, let’s use docker-compose. This will come useful later, when we will add another container (the database) linked with our app. For now it will run a single container. To do it all we need to do is create a [docker-compose.yml](https://github.com/federicomaffei/soul-compose/blob/faca99d02178407f3ffa9bfd932bcb8932dfd2e0/docker-compose.yml) file in our root folder:
...
In the next test we can dive into the thick of it: let’s say that we now want to get a nice and shiny page for the singer we just created, reading it from a data persistence system (we'll use MongoDB):
...
The tests will fail, the reason being our app is unable to connect to the database in its Docker environment at this point. So let’s take advantage of the features of docker-compose, and add a fully-featured MongoDB instance ready to be used, adding it to our [docker-compose.yml](https://github.com/federicomaffei/soul-compose/blob/2962d28e33366a92801f0ef2c18ef3dc7dd9f9db/docker-compose.yml) file.

Let’s go through the code. First and foremost, we added an entry for the MongoDB image to be built and ran:
...
To make sure that the app and MongoDB load up in the right order (mongo, then app) we can also specify a *depends_on* property, which means: the app will wait for Mongo to start, and then will be able to access it through the hostname *mongodb*.
...
-----
Thanks to [Stefano Ricciardi](https://twitter.com/stefanoric) and [John Catterfeld](https://twitter.com/jcatterfeld) for the proof-reading and feedback.
